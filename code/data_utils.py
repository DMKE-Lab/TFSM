...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    ...
    处理保存为一组三元组的KB数据。
        ...
        （b） 根据需要添加反向三元组。
        （c） 索引唯一实体和关系出现在知识库中。
        ：param raw_kb_path：原始kb三元组的路径。
        ：param train_path：序列集KB三元组的路径。
        ：param dev_path：开发集KB三元组的路径。
        ：param test_path：测试集KB三元组的路径。
        ：param add_reverse_relations：如果已设置，请向KB环境添加反向三元组。
