...
class AttentionSelectContext(nn.Module):
    ...
    def forward(self, left, right, mask_left=None, mask_right=None):
        ...
        # a =  self.Linear_tail(left)
        # b =  self.Linear_head(head_left)
        # left = a + b
...
class TransformerEncoder(nn.Module):
    # def __init__(self, model_dim=100, ffn_dim=800, num_heads=4, dropout=0.1, num_layers=6, max_seq_len=4,
    ...
    #     todo 添加
    #     self.time_embed = nn.Parameter(torch.rand(1, model_dim), requires_grad=True)
    ...
    # def forward(self, left, right, tem):
        ...
        # seq = torch.cat((left, rel_embed, right, tem), dim=1)
        # seq = torch.cat((left, rel_embed, tem, right), dim=1)

        pos = self.pos_embedding(batch_len=batch_size, seq_len=3)
        # pos = self.pos_embedding(batch_len=batch_size, seq_len=4)
        if self.with_pos:
            # 公式(6)
            output = seq + pos
        else:
